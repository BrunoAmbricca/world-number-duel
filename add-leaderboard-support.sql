...
CREATE POLICY IF NOT EXISTS "Allow read access on weekly_leaderboard" ON weekly_leaderboard
FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Allow read access on daily_leaderboard" ON daily_leaderboard
FOR SELECT USING (true);
...
-- Create a function to handle automatic refresh (can be called via cron job or scheduled task)
-- This should ideally be set up to run every hour or when needed
